"""Opt-in static TDA-BSE oracle against unmodified pinned QuAcK Fortran.
...
def run(upstream_dir, compiler, output=None):
    ...
    for path, digest in SOURCES.items():
        actual = hashlib.sha256((upstream_dir / Path(path).name).read_bytes()).hexdigest()
        if actual != digest:
    ...
    with tempfile.TemporaryDirectory(prefix="gradscf-quack-build-") as directory:
        ...
        command = [compiler, "-shared", "-fPIC", "-O2", "-I", str(upstream_dir),
                   str(upstream_dir / "RGW_phBSE_static_kernel_A.f90"),
                   str(upstream_dir / "phRLR_A.f90"), "-o", str(library)]
        subprocess.run(command, check=True, capture_output=True, text=True)
        lib = ctypes.CDLL(str(library))
        lib.rgw_phbse_static_kernel_a_.restype = None
        lib.phrlr_a_.restype = None
        rng = np.random.default_rng(83)
        factors = rng.normal(size=(5, 5, 5)) * .08
        factors = (factors + factors.transpose(0, 2, 1)) / 2
        screening = np.array([-1.1, -.6, .3, .8, 1.2])
        qp = screening + np.array([.03, .04, -.02, -.04, -.05])
        ...
        coulomb = lov.T @ lov
        squared, u = np.linalg.eigh(np.sqrt(gaps)[:, None] *
                                    (np.diag(gaps) + 4*coulomb) * np.sqrt(gaps)[None, :])
        ...
        lib.rgw_phbse_static_kernel_a_(ctypes.byref(eta), *dims, ctypes.byref(coupling),
                                       pointer(eri), pointer(omega), pointer(rho), pointer(ka))
        space = bse.make_bse_space(5, 2)
        state = build_static_screening(jnp.asarray(screening), jnp.asarray(factors),
                                       occupied=space.occupied, virtual=space.virtual)
        errors, matrices = {}, {}
        for singlet in (True, False):
            spin, drpa = ctypes.c_int(1 if singlet else 2), ctypes.c_int(0)
            bare = np.zeros((6, 6), order="F")
            lib.phrlr_a_(ctypes.byref(spin), ctypes.byref(drpa), *dims,
                         ctypes.byref(coupling), pointer(qp_f), pointer(eri), pointer(bare))
            expected = bare + ka
            operator = bse.build_tda_operator(jnp.asarray(qp), jnp.asarray(factors), space,
                                              state, singlet=singlet, block_size=2)
            actual = np.asarray(operator.apply(jnp.eye(6)))
            label = "singlet" if singlet else "triplet"
            errors[f"{label}_matrix_max_abs_hartree"] = float(np.max(np.abs(actual - expected)))
            errors[f"{label}_roots_max_abs_hartree"] = float(np.max(np.abs(
                np.linalg.eigvalsh(actual) - np.linalg.eigvalsh(expected))))
        ...
        metadata = {
            "upstream": "https://github.com/pfloos/QuAcK", "commit": COMMIT,
            "source_sha256": SOURCES, "seed": 83, "units": "Hartree",
            ...
            "oracle": "unmodified Fortran phRLR_A + RGW_phBSE_static_kernel_A; NumPy rho and RPA poles",
            ...
            "jax": jax.__version__, "numpy": np.__version__, "backend": jax.default_backend(),
            "dtype": "float64", "elapsed_seconds": time.perf_counter() - start,
            "command": "PYTHONPATH=src JAX_PLATFORMS=cpu JAX_ENABLE_X64=1 " + shlex.join([sys.executable, *sys.argv]),
            "errors": errors,
        }
        if output:
            output.parent.mkdir(parents=True, exist_ok=True)
            np.savez(output, qp_energy=qp, screening_energy=screening, factors=factors,
                     rpa_omega=omega, rho=rho, ka=ka, **matrices)
...
def main():
    ...
    output = Path(__file__).resolve().parents[1] / "bse/data/quack_static_seed83.npz"
    upstream_dir.mkdir(parents=True, exist_ok=True)
    for path in SOURCES:
        if not (upstream_dir / Path(path).name).exists():
            subprocess.run(["curl", "-L", "--fail", "--connect-timeout", "10", "--max-time", "30",
                            f"https://raw.githubusercontent.com/pfloos/QuAcK/{COMMIT}/{path}",
                            "-o", str(upstream_dir / Path(path).name)], check=True)
