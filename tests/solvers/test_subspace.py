...
from gradscf.solvers import EigenSolverConfig, EigenResponseConfig, LinearSolverConfig, solve_hermitian
...
def test_degenerate_projector_jvp_vjp_and_trace(method):
    ...
    def result(t):
        return _project(a+t*b, probe, config=cfg)
...
def test_matrix_free_projector_and_rhs_response():
    from gradscf.solvers import EigenSolverConfig, LinearOperator
    a, b, probe = problem()
    ...
    def calculate(t, rhs):
        ...
        return _project(op, rhs, config=cfg).projection
...
def test_cut_degenerate_boundary_has_invalid_response(method):
    ...
    fn = lambda t: _project(a+t*b, probe, config=cfg)
    result = fn(0.)
...
def test_internal_small_gaps_are_not_regularized():
    ...
    for gap in (0., 1e-12, 1e-6, .2):
        ...
        fn = lambda t: probe @ _project(base+t*b, probe, config=cfg).projection
        ad = jax.grad(fn)(0.)
        step = 1e-4
...
def test_full_subspace_is_identity_and_trace():
    ...
    def loss(t):
        out = _project(a+t*b, probe*(1+t), config=cfg)
        return jnp.sum(out.projection) + out.eigenvalue_sum
    out = _project(a, probe, config=cfg)
...
def test_private_frame_rotation_does_not_change_observable_response(angle):
    from gradscf.solvers.eigen.response import attach_spectral_response
    ...
    for splitting in (0., .2):
        ...
        def loss(t):
            y = attach_spectral_response(lambda v:(base+t*b)@v, x, config=cfg)
...
def test_projector_identities_and_vmap():
    ...
    a, b, _ = problem()
    cfg = EigenSolverConfig(nroots=2, adjoint_tol=1e-11)
    fn = lambda t: _project(a+t*b, jnp.eye(4), config=cfg).projection
    p, dp = jax.jvp(fn, (0.,), (1.,))
...
def test_unconverged_primal_and_response_linear_solve_are_invalid():
    from gradscf.solvers import EigenSolverConfig, LinearSolverConfig
    ...
    fn = lambda t: _project(a+t*jnp.outer(probe, probe), probe, config=cfg)
    out = fn(0.)
    ...
    def bad_response(t):
        out = _project(a+t*jnp.outer(probe, probe), probe,
...
def test_matrix_free_davidson_never_materializes_a_full_operator_block():
    ...
    fn = lambda t: probe@_project(build(t), probe, config=cfg).projection
...
def test_projector_boundary_tolerances_and_input_validation():
    ...
    out = _project(close, probe, config=cfg)
    assert out.converged and not out.response_valid
    assert _project(close, probe, config=cfg, gap_atol=1e-12, gap_rtol=0.).response_valid
    with pytest.raises(ValueError, match='boundary root'):
        _project(a, probe, config=EigenSolverConfig(nroots=2, max_subspace=2))
    with pytest.raises(ValueError, match='shape'):
        _project(a, jnp.ones(3), config=cfg)
    with pytest.raises(ValueError, match='positive'):
        _project(a, probe, config=cfg, gap_atol=0., gap_rtol=0.)
    with pytest.raises(NotImplementedError, match='real'):
        _project(a.astype(jnp.complex128), probe, config=cfg)
    bad = _project(a.at[0, 2].set(.1), probe, config=cfg)
...
def test_rank_one_reduces_to_isolated_root_response():
    ...
    new = lambda t: probe@_project(a+t*b, probe, config=cfg).projection
...
def test_unconverged_guard_root_invalidates_even_a_converged_selected_root():
    ...
    out = _project(a, jnp.ones(4), config=cfg,
                                   initial_vectors=jnp.eye(4)[:, :2])
    assert out.residual_norms[0] == 0
    assert out.raw_residual_norms[1] > cfg.atol
...
def test_invalid_boundary_retains_primal_with_direct_response_solver():
    ...
    def result(t):
        return _project(a+t*b, probe, config=cfg, linear_config=linear)
    out = result(0.)
    assert out.converged and not out.response_valid
...
def test_bounded_davidson_restart_with_guard_root():
    ...
    out = _project(a, probe, config=EigenSolverConfig(
...
def test_default_start_explores_disconnected_invariant_sectors(size):
    ...
    out = _project(a, jnp.ones(size), config=EigenSolverConfig(nroots=2))
    assert out.converged and not out.response_valid
