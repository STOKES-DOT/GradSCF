...
def main():
    ...
    diagonal = jnp.array([1., 1., 3., 5.])
    perturbation = jnp.array([[.2, .6, .4, .1], [.6, -.3, .2, .3],
                              [.4, .2, .1, -.2], [.1, .3, -.2, .4]])
    probe = jnp.array([.3, .4, .5, .2])
    config = EigenSolverConfig(nroots=2, atol=1e-11, adjoint_tol=1e-11)

    def solve(t, cfg=config):
        op = LinearOperator((4, 4), diagonal.dtype,
            lambda x: diagonal*x+t*(perturbation@x),
            diagonal=diagonal+t*jnp.diag(perturbation))
        return solve_spectral_projector(op, probe, config=cfg)
    ...
    value, backward = jax.jit(jax.value_and_grad(loss))(0.)
    forward = jax.jvp(loss, (0.,), (1.,))[1]
    step = 1e-4
    ...
    cut = solve(0., replace(config, nroots=1))
    assert cut.converged and not cut.response_valid
    print(json.dumps(dict(
        machine=platform.machine(), jax=jax.__version__, devices=[str(d) for d in jax.devices()],
        dtype=str(diagonal.dtype), dimension=4, rank=2, eigenvalue_sum=float(result.eigenvalue_sum),
        boundary_gap=float(result.boundary_gap), residual_norms=np.asarray(result.residual_norms).tolist(),
        loss=float(value), jvp=float(forward), vjp=float(backward),
        finite_difference=float(finite_difference), step=step,
        absolute_error=float(abs(backward-finite_difference)),
        cut_cluster_status=int(cut.status), elapsed_seconds=time.perf_counter()-start,
    ), indent=2))
