...
class CartesianBasis:
    ...
    def __post_init__(self) -> None:
        ...
        geometry_is_traced = _contains_jax_tracer(
            [value for ao in self.aos for value in (ao.center, ao.exponents, ao.coefficients)]
            + [value for shell in self.shells for value in (shell.center, shell.exponents, shell.coefficients)]
        )
        ...
        if self.ao_exponents_padded is None or self.ao_coefficients_padded is None:
            ...
            elif geometry_is_traced:
                exp_pad = jnp.stack([jnp.pad(ao.exponents, (0, max_nprim-ao.exponents.shape[0]))
                                     for ao in self.aos])
                coeff_pad = jnp.stack([jnp.pad(ao.coefficients, (0, max_nprim-ao.coefficients.shape[0]))
                                       for ao in self.aos])
            else:
        ...
        if self.shell_exponents_padded is None or self.shell_coefficients_padded is None:
            ...
            if nshell == 0 or max_shell_nprim == 0:
                ...
                exp_pad = jnp.stack([jnp.pad(sh.exponents, (0, max_shell_nprim-sh.exponents.shape[0]))
                                     for sh in self.shells])
                coeff_pad = jnp.stack([jnp.pad(sh.coefficients, (0, max_shell_nprim-sh.coefficients.shape[0]))
                                       for sh in self.shells])
            else:
